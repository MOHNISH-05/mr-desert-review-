...
const stories: Record<string, { kicker: string; title: string; copy: string; images: string[] }> = {
  "mr-desert": {
    kicker: "01 · The Host",
    ...
    images: ["/images/dheeraj/mr-desert-alley.webp", "/images/dheeraj/dheeraj-purohit.webp", "/images/dheeraj/camel-safari.webp"],
  },
  "elite-castle": {
    kicker: "02 · The Stay",
    ...
    images: ["/images/official/elite-castle-story.webp", "/images/dheeraj/elite-castle-family-06.webp", "/images/dheeraj/elite-castle-family-22.webp"],
  },
  "happy-adventure": {
    kicker: "03 · The Camp",
    ...
    images: ["/images/dheeraj/happy-camp-night.webp", "/images/dheeraj/happy-camp-day.webp", "/images/dheeraj/camel-safari.webp"],
  },
  "tour-planner": {
    ...
    images: ["/images/dheeraj/dheeraj-purohit.webp", "/images/dheeraj/camel-safari.webp", "/images/dheeraj/mr-desert-alley.webp"],
  },
};

const shortNames: Record<string, string> = {
  "mr-desert": "Mr. Desert",
  "elite-castle": "Elite Castle",
  "happy-adventure": "Happy Adventure",
  "tour-planner": "Elite India Tour",
};

function StoryImage({ images, alt, kicker }: { images: string[]; alt: string; kicker: string }) {
  const [index, setIndex] = useState(0);

  useEffect(() => {
    if (images.length < 2) return;
    const timer = window.setInterval(() => setIndex((c) => (c + 1) % images.length), 4000);
  ...
  return (
    <div className="group relative overflow-hidden aspect-[4/3] rounded-2xl bg-desert-200 shadow-luxury">
      ...
      <span className="absolute top-5 left-5 flex items-center gap-1.5 rounded-full border border-white/20 bg-black/40 px-3 py-1.5 text-[10px] uppercase tracking-[0.25em] text-white backdrop-blur-sm">
        ...
        {images.map((_, dot) => (
          <span
            ...
            className={`h-1 rounded-full transition-all duration-500 ${
              dot === index ? "w-6 bg-desert-400" : "w-1 bg-white/40"
            }`}
          />
        ))}
      </div>
