...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const review = await api.reviews.get(Number(id));
    return {
      title: `${review.title || "Guest Story"} | Mr. Desert Reviews`,
      description: review.content,
    };
  } catch {
    const fallback = FALLBACK_REVIEWS.find((r) => r.id === Number(id));
    if (fallback) {
      return {
        title: `${fallback.title || "Guest Story"} | Mr. Desert Reviews`,
        description: fallback.content,
      };
    }
    return { title: "Guest Story | Mr. Desert Reviews" };
  }
}
  ...
  return (
    <article className="min-h-screen bg-[#f7f3ed] pb-24 pt-28">
      <div className="container max-w-6xl">
        <Link href="/all-reviews" className="mb-8 inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-desert-700">
        ...
        <header className="grid items-end gap-10 lg:grid-cols-[1.25fr_.75fr]">
          <div className="relative aspect-[16/10] overflow-hidden rounded-2xl shadow-xl">
            <img src={cover} alt={review.title || "Guest story"} className="h-full w-full object-cover" />
